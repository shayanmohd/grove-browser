...
const source = options.restore
  ...
  : resolve(dirname(fileURLToPath(import.meta.url)), "../skills/kamapathy");
...
if (
  ...
  (dirname(source) !== dirname(target) ||
    !basename(source).startsWith(".kamapathy-backup-"))
)
  throw new Error(
    "Restore accepts only a Kamapathy backup beside the destination.",
  );
await lstat(join(source, "SKILL.md"));
await lstat(join(source, "scripts/kamapathy.mjs"));
await mkdir(dirname(target), { recursive: true });
const staging = join(dirname(target), `.kamapathy-install-${randomUUID()}`);
const backup = join(
  dirname(target),
  `.kamapathy-backup-${Date.now()}-${randomUUID().slice(0, 8)}`,
);
let backedUp = false;
try {
  ...
  process.stdout.write(
    `${options.restore ? "Restored" : "Installed"} Kamapathy skill: ${target}\n`,
  );
  if (backedUp)
    process.stdout.write(
