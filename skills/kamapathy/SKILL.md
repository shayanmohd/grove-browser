...
Snapshots cover the top document, not iframes, shadow roots, PDF internals, or canvas controls. Use `screenshot TAB_ID ./page.png` when visual inspection helps; it saves a viewport PNG and refuses to overwrite a file. Screenshots require a shown, unminimized Kamapathy window. If `screenshot_unavailable` is returned, use a snapshot when sufficient or ask the user to restore Kamapathy before retrying. Report unsupported interactions rather than silently switching to another browser or broader access.
