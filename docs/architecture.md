...
The Debian packager needs project homepage metadata and a maintainer. The config declares Grove contributors as maintainer, and `package.json` declares the repository homepage. Update that field when packaging a fork. Keeping the value in package metadata also avoids shell-specific parsing of dotted command-line overrides.
