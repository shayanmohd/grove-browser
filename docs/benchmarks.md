...
Kamapathy's full snapshot output uses **1,605 tokens** across four captured local fixture pages under the `o200k_base` tokenizer. These observations were recorded on September 11, 2026, using Kamapathy 0.1.0 on an Apple Silicon Mac running macOS 15.7.2 and Electron 44.2.0.
...
For an interactive review, run `node scripts/review-session.mjs` in a terminal. Use its printed local origin with the Kamapathy skill, setting `KAMAPATHY_USER_DATA` to its printed profile folder. Press Enter in the review session to close the temporary browser and fixture.
...
Kamapathy provides a bounded summary of the top document with short control refs, exact select option values, control states, and current field values. It hides password, one-time code, and payment card values. The compact default and scoped observations can return less text than the full snapshots measured here. Lower output size alone does not establish completeness or success on a particular task.
